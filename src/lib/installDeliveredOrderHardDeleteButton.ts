...
function buildHardDeleteButton(anchorButton: HTMLButtonElement): HTMLButtonElement {
  ...
  button.title = 'يحذف الأوردر نهائياً مهما كانت حالته ويعيد قطع الغيار المسحوبة إلى المخزون';
  ...
  button.addEventListener('click', async event => {
    ...
    const order = resolveSelectedOrder(anchorButton);
    const orderId = String(order?.id || order?.orderNumber || '').trim();
    if (!order || !orderId) {
      window.alert('تعذر تحديد أمر الصيانة المحدد. افتح الأوردر مرة أخرى ثم حاول الحذف.');
      return;
    }

    const firstConfirm = window.confirm(`تحذير: سيتم حذف أمر الصيانة [${orderId}] نهائياً مهما كانت حالته، وحذف السجلات المرتبطة به، وإرجاع أي قطع غيار مسحوبة إلى المخزون.\n\nهل تريد الاستمرار؟`);
    if (!firstConfirm) return;
    const secondConfirm = window.confirm(`تأكيد أخير: حذف [${orderId}] نهائياً لا يمكن التراجع عنه.\n\nاضغط موافق لتنفيذ الحذف الآن.`);
    ...
    try {
      ...
      window.alert(error?.message || 'حدث خطأ أثناء الحذف النهائي للأوردر.');
...
function ensureButton() {
  scheduled = false;
  if (busy) return;
  const anchorButton = findDeliveryActionButton();
  const existing = document.querySelector(`[${BUTTON_DATA_ATTRIBUTE}="1"]`);
  if (!anchorButton) { existing?.remove(); return; }
  const container = anchorButton.parentElement;
  if (!container) return;
  if (existing && existing.parentElement === container) return;
  existing?.remove();
  anchorButton.insertAdjacentElement('afterend', buildHardDeleteButton(anchorButton));
}
...
if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', scheduleEnsureButton, { once: true });
  else scheduleEnsureButton();
